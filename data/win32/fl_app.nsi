...
# substitute your application name for instances of FLCLUSTER

# Variables
!define FLCLUSTER_DESCRIPTION "${FLCLUSTER_NAME} ${FLCLUSTER_VERSION}"
!define FLCLUSTER_STRING "${FLCLUSTER_NAME}-${FLCLUSTER_VERSION}"

!define PRODUCT_BINARY "${FLCLUSTER_BINARY}"
!define PRODUCT_NAME "${FLCLUSTER_NAME}"
!define PRODUCT_VERSION "${FLCLUSTER_VERSION}"
!define PRODUCT_STRING "${FLCLUSTER_STRING}"
!define PRODUCT_DESCRIPTION "${FLCLUSTER_DESCRIPTION}"
...
VIAddVersionKey FileDescription "${FLCLUSTER_DESCRIPTION} installer"
...
Section -install
    ...
    WriteRegStr HKLM "${REG_UNINSTALL_PATH}" "Publisher" "FLCLUSTER developers"
...
Section "Start Menu Shortcuts"
	...
	CreateShortCut "${SM_PATH}\${FLCLUSTER_NAME}.lnk" "$INSTDIR\${FLCLUSTER_BINARY}" "" "$INSTDIR\${FLCLUSTER_BINARY}" 0
    CreateShortCut "${SM_PATH}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
SectionEnd

Section "Desktop Shortcuts"
	CreateShortCut "$DESKTOP\${FLCLUSTER_DESCRIPTION}.lnk" "$INSTDIR\${FLCLUSTER_BINARY}" "" \
		"$INSTDIR\${FLCLUSTER_BINARY}" 0
SectionEnd

# This is unselected by default
Section /o "Quick Launch Shortcuts"
	CreateShortCut "$QUICKLAUNCH\${FLCLUSTER_DESCRIPTION}}.lnk" "$INSTDIR\${FLCLUSTER_BINARY}" "" \
		"$INSTDIR\${FLCLUSTER_BINARY}" 0
SectionEnd
...
# Remove files and uninstaller
	Delete /REBOOTOK $INSTDIR\${FLCLUSTER_BINARY}
    Delete /REBOOTOK $INSTDIR\uninstall.exe

# Remove shortcuts, if any
    Delete "${SM_PATH}\*.*"
	Delete "$DESKTOP\${FLCLUSTER_DESCRIPTION}.lnk"
	Delete "$QUICKLAUNCH\${FLCLUSTER_DESCRIPTION}.lnk"
